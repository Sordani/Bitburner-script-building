...
import { buildramNetwork, solveGrow, isPrepped, prepTarget, countPrograms, Weight, getServerList, updateTarget, getBestTarget } from "slibs.js";
...
export async function main(ns) {
    ...
    let ramNetwork = [];
    const values = {
        totalPossibleThreads: 0,
        optimalTarget: 'n00dles',
        maxServerRamSize: 0,
        minServerRamSize: Infinity,
    }
    //values.optimalTarget = getBestTarget(ns, updateTarget(ns)); //this gets the best target
    values.optimalTarget = updateTarget(ns).sort((a, b) => b.score - a.score)[1].name    //this will get the second best target since we have earlyhacktemplate running everywhere else against the best.
    ...
    if (!isPrepped(ns, values.optimalTarget)) {
        ...
        ramNetwork = [];
        values.minServerRamSize = Infinity;
        values.maxServerRamSize = 0;
	...
	let greed = 0.20
    let hackSet = server.moneyMax * greed;
    ...
    while (true) {
        ...
        ns.print(`Expected yield: \$${ns.formatNumber(hackSet * (1000 / (wkTime + 20 + spacer)), 2)} per second`); 
