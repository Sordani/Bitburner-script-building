export class Business {
	...
	async enronFraudPrep() {
		...
		if (this.stage[1] == 0) {
			...
			for (let i = 0; i < 10; i++) {
				if (this.ns.corporation.getUpgradeLevelCost(this.lvlUps[6]) > wilFunds) { break; }
				wilFunds -= this.ns.corporation.getUpgradeLevelCost(this.lvlUps[6]);
				this.ns.corporation.levelUpgrade(this.lvlUps[6]); //wilson's level 10
			}
			for (let i = 0; i < 50; i++) {
				if (this.ns.corporation.getUpgradeLevelCost[this.lvlUps[8]] > abcFunds) { break; }
				abcFunds -= this.ns.corporation.getUpgradeLevelCost[this.lvlUps[8]];
